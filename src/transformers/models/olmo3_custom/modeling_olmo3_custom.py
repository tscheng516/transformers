...
from .configuration_olmo3_custom import Olmo3CustomConfig


@use_kernel_forward_from_hub("RMSNorm")
class Olmo3CustomRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps: float = 1e-6) -> None:
        """
        Olmo3CustomRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class Olmo3CustomAttention(nn.Module):
...
class Olmo3CustomMLP(nn.Module):
...
class Olmo3CustomDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Olmo3CustomConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class Olmo3CustomRotaryEmbedding(nn.Module):
    inv_freq: torch.Tensor  # fix linting for `register_buffer`

    def __init__(self, config: Olmo3CustomConfig, device=None):
        super().__init__()
...
@auto_docstring
class Olmo3CustomPreTrainedModel(PreTrainedModel):
    config: Olmo3CustomConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Olmo3CustomDecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": Olmo3CustomDecoderLayer,
        "attentions": Olmo3CustomAttention,
    }


@auto_docstring
class Olmo3CustomModel(Olmo3CustomPreTrainedModel):
    def __init__(self, config: Olmo3CustomConfig):
...
@auto_docstring
class Olmo3CustomForCausalLM(Olmo3CustomPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = Olmo3CustomModel(config)
        self.vocab_size = config.vocab_size
