...
"""Testing suite for the PyTorch Olmo3Custom model."""
...
if is_torch_available():
    ...
    from transformers import (
        Olmo3CustomForCausalLM,
        Olmo3CustomModel,
    )
    from transformers.models.olmo3_custom.modeling_olmo3_custom import Olmo3CustomRotaryEmbedding


class Olmo3CustomModelTester(CausalLMModelTester):
    if is_torch_available():
        base_model_class = Olmo3CustomModel


@require_torch
class Olmo3CustomModelTest(CausalLMModelTest, unittest.TestCase):
    all_model_classes = (Olmo3CustomModel, Olmo3CustomForCausalLM) if is_torch_available() else ()
    test_all_params_have_gradient = False
    model_tester_class = Olmo3CustomModelTester
    ...
    _torch_compile_train_cls = Olmo3CustomForCausalLM if is_torch_available() else None

    @parameterized.expand([("linear",), ("dynamic",), ("yarn",)])
    def test_model_rope_scaling_from_config(self, scaling_type):
        config, _ = self.model_tester.prepare_config_and_inputs_for_common()

        # Rope only gets applied to full attention layers in Olmo3Custom, so make all layers full attention.
    ...
    def test_model_rope_scaling_frequencies(self):
        ...
        # Parent test class's attempt to find Olmo3Custom rope fails, so we pass here explicitly.
        rope_class = Olmo3CustomRotaryEmbedding
...
class Olmo3CustomIntegrationTest(unittest.TestCase):
    ...
    def test_model_7b_logits(self):
        ...
        model = Olmo3CustomForCausalLM.from_pretrained("shanearora/2025-sep-a-base-model").to(
    ...
    def test_model_7b_greedy_generation(self):
        ...
        model = Olmo3CustomForCausalLM.from_pretrained("shanearora/2025-sep-a-base-model", device_map="auto")
    ...
    def test_export_static_cache(self):
        ...
        olmo3_custom_model = "shanearora/2025-sep-a-base-model"

        tokenizer = AutoTokenizer.from_pretrained(olmo3_custom_model, pad_token="</s>", padding_side="right")
        ...
        model = Olmo3CustomForCausalLM.from_pretrained(
            olmo3_custom_model,
            device_map=device,
            dtype=dtype,
